...
> **You are on `week-08-gate`: the starting line for the last week.**
>
> Weeks 1-7 are complete and passing. This week you build `evals/run_evals.py`
> (the gate), add judge cases to `evals/cases.json`, wire the gate into CI,
> rehearse a rollback, and read `deploy/PORTABILITY.md` and
> `deploy/KUBERNETES.md`. `evals/judge.py` is given — read it, it needs no
> changes.
>
> Read **[`guide/week-08.md`](guide/week-08.md)**, then run
> `make check-week-08` until it is green.
>
