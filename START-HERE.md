...
The project is built **layer by layer, one branch per week**. Every week is a
pair:

```
week-01-package    <- start here. Working code, plus what you build.
week-01-solution   <- the answer key for week 1
...
**Start a week by checking out its branch.** Everything from previous weeks is
complete and passing, so you never debug someone else's half-finished code:
...
Read that week's guide in `guide/`, build until the checkpoint is green, then
...
Each file you need to build carries a header comment telling you exactly what
goes in it and why. The checkpoint failures name the fix, not just the problem.
